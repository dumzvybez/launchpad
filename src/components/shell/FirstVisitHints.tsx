 ...
 * auto-dismissing (~3 second) hint at the top describing what the view is for.
...
export function FirstVisitHints() {
  ...
  // Show the Command Palette tip once (persistent until dismissed).
  useEffect(() => {
    try {
      const seen = window.localStorage.getItem(CMDK_TIP_SEEN);
      if (seen !== "1") {
        // Small delay so it doesn't overlap with a view hint.
        const t = setTimeout(() => setShowCmdkTip(true), 1500);
        return () => clearTimeout(t);
      }
    } catch {
      // ignore
  ...
  return (
    <>
      {/* Ephemeral view hint — auto-dismissing, top-center */}
      {hint && (
        <div
          className="fixed top-16 left-1/2 -translate-x-1/2 z-[90] px-4 py-2 rounded-xl glass-elevated border border-primary/30 shadow-lg flex items-center gap-2 text-xs text-foreground max-w-[90vw]"
